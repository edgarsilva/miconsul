...
func (s *service) HandleLogtoSignin(c fiber.Ctx) error {
	...
	if err != nil {
		...
		return s.Redirect(c, "/login?logto_error=session")
	}
...
func (s *service) HandleLogtoCallback(c fiber.Ctx) error {
	...
	if err != nil {
		...
		return s.Redirect(c, "/login?logto_error=id_token")
	}

	logtoUser, err := NewLogtoUser(claims)
	if err != nil {
		log.Error("failed to build user from id token claims:", err)
		return s.Redirect(c, "/login?logto_error=claims")
	}
	...
	if err != nil {
		log.Error(err)
		return s.Redirect(c, "/login?logto_error=user_sync")
	}
