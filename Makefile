...
test/coverage: ## Coverage
	@mkdir -p coverage
	@rm -f coverage/all.out coverage/all.filtered.out coverage/summary.filtered.txt coverage/pkg_coverage.txt coverage/service_pkg_coverage.txt coverage/cmd_app.out coverage/cmd_seed.out coverage/cmd_coverage.txt
	...
	@go test ./cmd/app -covermode=atomic -coverprofile=coverage/cmd_app.out
	@go test ./cmd/seed -covermode=atomic -coverprofile=coverage/cmd_seed.out
	...
	@printf "\033[32m✅ Coverage total: %s (generated files excluded; internal KPI)\033[0m\n" "$$(awk '/^total:/{print $$NF}' coverage/summary.filtered.txt)"
